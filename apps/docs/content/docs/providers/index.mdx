...
<Tabs items={['OpenAI / Google / xAI', 'Anthropic', 'Ollama']}>
  ...
  <Tab value="Ollama">
    ...
    ```
    <Callout type="info">
    **No additional SDK needed!** Ollama uses native `fetch` for local inference.
    </Callout>
  </Tab>
</Tabs>
...
| OpenRouter | `@yourgpt/llm-sdk/openrouter` | `openai` | `openrouter('anthropic/claude-3.5-sonnet')` |
| Fireworks | `@yourgpt/llm-sdk/fireworks` | `openai` | `fireworks('accounts/fireworks/models/llama-v3p1-70b-instruct')` |
...
| OpenRouter | Varies | Any model | $ | Model flexibility, single key |
| Fireworks | Ultra Fast | Very Good | $ | Open-source models, cost savings |
