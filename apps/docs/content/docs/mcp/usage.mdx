...
Connects to an MCP server and automatically registers tools with `CopilotProvider`.
...
<Callout type="info">
When using `createMCPClient` directly, you'll need to manually pass tools to your runtime or register them with `CopilotProvider`.
</Callout>
